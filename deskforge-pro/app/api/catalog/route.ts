...
export async function GET(){let u=await requireUser(),items=await prisma.serviceCatalogItem.findMany({where:{tenantId:u.tenantId,isActive:true},orderBy:[{category:'asc'},{name:'asc'}]});return NextResponse.json({items,total:items.length})}
