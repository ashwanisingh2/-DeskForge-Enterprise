...
export async function GET(_:NextRequest,ctx:{params:Promise<{id:string}>}){let {id}=await ctx.params;if(isLocalDemo()){let d=demoChangeRecords.find(c=>c.id===id);return d?NextResponse.json(d):NextResponse.json({error:'NOT_FOUND'},{status:404})}let u=await requireUser('change:manage'),change=await prisma.changeRequest.findFirst({where:{id,tenantId:u.tenantId},include:{requester:true,approvals:true}});return change?NextResponse.json(change):NextResponse.json({error:'NOT_FOUND'},{status:404})}
